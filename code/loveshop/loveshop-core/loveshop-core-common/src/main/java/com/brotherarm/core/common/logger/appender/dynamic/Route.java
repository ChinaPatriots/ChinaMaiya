 ...
 */
//@Plugin(name = "DRoute", type = "Core", printObject = true, deferChildren = true)
@Plugin(name = "DRoute", category = "Core" ,printObject = true, deferChildren = true)
public final class Route {
     ...
     */
    @PluginFactory
    public static Route createRoute(@PluginAttribute("ref") final String appenderRef,
    								@PluginAttribute("key") final String key,
                                    @PluginNode final Node node) {
        if (node != null && node.hasChildren()) {
            for (@SuppressWarnings("unused") final Node child : node.getChildren()) {
            }
            if (appenderRef != null) {
